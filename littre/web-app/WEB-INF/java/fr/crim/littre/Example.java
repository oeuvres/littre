...
import fr.crim.littre.lucene.LittreAnalyzer;


/**
 * Reprend la classe IndexEntry en ajoutant les exemples d'utilisation
 * 
 * @author Ginka
 * @author frederic.glorieux@fictif.org
 * @author Kerroua Sofiane
 */
public class Example extends Thread {
	/**  */
	
	/** Expression régulière qui capture les exemples d'utilisation */
	 ...
	 * @throws IOException 
	 */
	public Example(Writer out) throws IOException {
	 ...
	 * @throws IOException 
	 */
	public Example(PrintWriter out) throws IOException {
		this(out, null, null);
	}
	/**
	 ...
	 * @throws IOException 
	 */
	public Example(PrintWriter aOut, File aXmlDir, File aIndexDir) throws IOException {
	...
	public static void parse(File xml) throws XMLStreamException, TransformerException, CorruptIndexException,
		...
		// texte en cours
		String text = null;
		// Contient les exemples d'utilisation
		String exemple;
		...
		String id;
		int pos;
		// Indique si la balise dictScrap, contient une balise oVar
		boolean b = false;
		...
		while (reader.hasNext()) {
			...
			if (ev.isStartElement()) {
				...
				// Permet de remplacer les balises oVar par @, pour repérer la vedette
				else if ("oVar".equals(name)) dictScrap.append("@");
			}
			...
			if (ev.isEndElement()) {
				...
				// Permet de remplacer les balises oVar par @, pour repérer la vedette
				else if ("oVar".equals(name)) {
					dictScrap.append("@");
					b = true; // La balise dictScrap contient une balise oVar
				}
				// On traite seulement les noeud dictScrap qui contiennent un noeud oVar
				else if ("dictScrap".equals(name) && b) {
					// Conversion pour appliquer l'expression régulière
					exemple = dictScrap.toString();
					match = pattern.matcher(exemple);
					// Au sein d'une même définition, il y a plusieurs exemples d'utilisation
					while ( match.find() ) { 
						// On indexe les exemples d'utilisation avec une limite sur la taille pour atténuer le bruit
						if(match.group(1).length() < 152) doc.add(new Field("example", match.group(1).replaceAll("@", ""), Field.Store.YES, Field.Index.ANALYZED));
					}	
					b = false;
				}
	...
	public static void main(String[] args) throws Exception {
		Thread task;
		if (args.length == 2) task=new Example(null, new File(args[0]), new File(args[1]));
		else task=new Example();
