...
var Sortable = {
  ...
  /**
   * Sort a prepared table. The trick is to modify the String.toString() method
  ...
  create: function(table) {
    ...
    for (var i=0;i<firstRow.cells.length;i++) {
      ...
      if (cell.className.indexOf("unsort") != -1) continue;
      cell.table=table;
   ...
   */
  key: function(text) {
    if (typeof text != 'string') text=text.innerText||text.textContent;
    text=Sortable.trim(text);
    // num TODO '.'
    if (text.match(/^-?[\.\d]+/)) {
      text=text.replace(/[^0-9\.\-]/g, '');
      // prefix with a char befor '0' and not stripped by defaut sort compare
      if (text.charAt(0) == '-') return "'"+ (10000000000 + parseFloat(text));
      if(text.indexOf('.')>-1) {
        text=(""+parseFloat(text)+"0000000000").substring(0, 10);
        return text;
      }
